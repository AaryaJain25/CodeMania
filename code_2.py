...
started = False # Add a new variable to check if the quiz has started

def displayStartButton():
    global started
    while not started:
        success, img = cap.read()
        img = cv2.flip(img, 1)
        img, _ = cvzone.putTextRect(img, "CodeMania", [550, 100], 2, 2, offset=50, border=5)
        img, bbox1 = cvzone.putTextRect(img, "START", [580, 350], 2, 2, offset=50, border=5)
...
displayStartButton() # Call the function to display the start button
...
while True:
    ...
    if qNo < qTotal:
        ...
        if hands:
            lmList = hands[0]['lmList']
            cursor = lmList[8]
            print(lmList[8]) # Add this line
            print(lmList[12]) # Add this line
            length, _ = detector.findDistance(lmList[8], lmList[12])
            print(length)
            if length < 35:
